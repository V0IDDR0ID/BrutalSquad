...
func load_mod_json(path):
	...
	if json.open(path, File.READ) == OK:
		...
		elif not (mod.result is Dictionary):
...
func check_mod_json(m: Dictionary)->Array:
	var errs = []
	if not m.has_all(["name", "author", "version", "description"]):
		errs.append("Missing one or more of \"name\", \"author\", \"version\", \"description\" properties")
	if m.has("dependencies") and not (m["dependencies"] is Array):
	...
	if m.has("priority") and ( not (m.priority is float) or m.priority < 0):
...
func load_mod_info(path: String)->Dictionary:
	...
	if not mod_json.has("error"):
		...
		mod_log("     - Mod path: " + path_wrap(path), "Mod Loader")
...
func init_mods()->Array:
	...
	if MULTI_ROOT == true:
		if not dir.dir_exists("user://mods"):
...
func _init():
	...
	for key in mod_dep_names.keys():
		if not (key in mod_names):
			missing_dep_names.append(key)

	if not missing_dep_names.empty():
	...
	if not quit_game:
		...
		for mod in MODS:
			...
			if mod["name"] in init_scripts.keys():
				...
				n.initialized = true
			if bad_files.empty() and not mod.has("error"):
				add_child(n)
				loads += 1
...
func mod_log_dir_init(force = false)->void :
	...
	if not mod_log_dir.dir_exists(mod_log_dir_path):
		if not mod_log_dir.make_dir(mod_log_dir_path):
...
func add_menu_version():
	var margin = MarginContainer.new()
	margin.add_constant_override("margin_left", 100)
	...
	Global.get_node("Menu").add_child(margin)
"""
