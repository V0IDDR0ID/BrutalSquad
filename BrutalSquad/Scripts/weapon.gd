...
var flashlight
var glob
onready var tactic_ammo = $Player_Weapon/Player_Weapon/Skeleton/TacticAmmo
onready var ammo_timer = $Player_Weapon/Player_Weapon/AmmoTimer
...
const PILLS = preload("res://Entities/Physics_Objects/pills.tscn")
const EXPLOISONCORE = preload("res://Entities/Bullets/Self_Destruct_Explosion.tscn")
var recoil = 0
var audio
...
func _input(event):
	...
	if event is InputEventMouseMotion and player and current_weapon != null:
		if Input.is_action_pressed("reload"):
			if player and Global.implants.head_implant.noir_vision:
				tactic_ammo.text = str(magazine_ammo[current_weapon])+"/"+str(ammo[current_weapon])
				tactic_ammo.show()
...
func _process(delta)->void :
	...
	if player:
		...
		if Input.is_action_pressed("mouse_1") and reload_timer.is_stopped() and current_weapon != null:
			if magazine_ammo[current_weapon] > 0 or Global.implants.arm_implant.zippy_fingers:
		...
		if Input.is_action_just_pressed("Tertiary_Weapon") and Global.implants.arm_implant.augarm:
			if current_weapon == weapon3:
				return 
			$Switch.play()
			anim.stop()
			anim.play("Nogun", - 1, 100)
...
func shoot()->void :
	...
	if magazine_ammo[current_weapon] > 0 and player and current_weapon != W_TRANQ and current_weapon != W_BLACKJACK and current_weapon != W_PISTOL and timer.is_stopped() and current_weapon != W_SILENCED_SMG and current_weapon != W_MAUSER and current_weapon != W_NAMBU and current_weapon != W_RADIATOR and current_weapon != W_FLASHLIGHT and current_weapon != W_CANCER and current_weapon != W_ROD and current_weapon != W_NAILER:
		...
		for body in nearby:
			if body.has_method("alert"):
				body.alert(global_transform.origin)
	if Global.implants.arm_implant.zippy_fingers and player:
			zippy()
			magazine_ammo[current_weapon] = MAX_MAG_AMMO[current_weapon]
...
func set_weapon(weapon_index):
	if player and not orb:
		...
		elif current_weapon == weapon3:
			weapon3 = weapon_index
