...
func _ready():
	...
	if not GLOBAL.implants.head_implant.nightmare and not GLOBAL.implants.head_implant.nightvision and not GLOBAL.implants.head_implant.noir_vision:
		$NV.queue_free()
		Global.border.show()
	...
	if GLOBAL.implants.head_implant.noir_vision:
		Global.border.hide()
		$NV.show()
		shader_screen.material.set_shader_param("noir_vision", true)
...
func set_move_speed():
	...
	if weapon.weapon2 != null:
		...
		pass
	if weapon.weapon3 != null:
		move_speed -= weapon.weight[weapon.weapon3]
	else:
		pass
	if move_speed < 5:
		run_acceleration = 15
...
func _physics_process(delta):
	...
	if drugged and not Global.implants.head_implant.pills_filter:
