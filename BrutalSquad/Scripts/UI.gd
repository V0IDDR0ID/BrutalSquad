...
func set_boss_UI(boss_texture: String):
	$Ammovbox/BossHBoxContainer/Boss_Thumbnail.texture = load(boss_texture)
	$Ammovbox/BossHBoxContainer.show()

func update_boss_UI(health_value: String):
	$Ammovbox/BossHBoxContainer/Boss_Health.text = health_value
