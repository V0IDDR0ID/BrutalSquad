...
var meatsound = preload("res://Entities/Decals/Meat_Sound.tscn")
...
func _ready():
	...
	darktimer = Timer.new()
	add_child(darktimer)
	darktimer.wait_time = 1
	darktimer.one_shot = true
	darktimer.connect("timeout", self, "dark_timeout")
	crueltytimer = Timer.new()
	add_child(crueltytimer)
	crueltytimer.wait_time = 5
	crueltytimer.one_shot = true
	crueltytimer.connect("timeout", self, "cruelty_timeout")
...
func _physics_process(delta):
	...
	if alerted and not dead:
		if npc_name == "Lifeman":
			Global.UI.set_boss_UI("res://Textures/UI/thumbnails/lifeboss.png")
			Global.UI.update_boss_UI(str(int(health)))
...
func dark_timeout():
	var dark_misery = DARK_DESTRUCT.instance()
	add_child(dark_misery)
	dark_misery.global_transform.origin = body.global_transform.origin
	damage(500, Vector3.ZERO, body.global_transform.origin, Vector3.ZERO)
func cruelty_timeout():
	dead = true
	var self_explosion = SELF_DESTRUCT_EXPLOSION.instance()
	add_child(self_explosion)
	self_explosion.global_transform.origin = body.global_transform.origin
	$Body/SFX/Exploison.play()
	damage(500, Vector3.ZERO, body.global_transform.origin, Vector3.ZERO)
